...
class CustoOperacionalController extends BaseController
{
    ...
    {
        $params = [];
        $dtIniStr = $request->get('dtIni') ?? '01/' . DateTimeUtils::incMes(new \DateTime(), -12)->format('m/Y');
        $dtFimStr = $request->get('dtFim') ?? DateTimeUtils::incMes(new \DateTime(), -1)->format('t/m/Y');
        $params['dtIni'] = $dtIniStr;
        $params['dtFim'] = $dtFimStr;

        try {

            $dtIni = DateTimeUtils::parseDateStr($dtIniStr);
            $dtFim = DateTimeUtils::parseDateStr($dtFimStr);
