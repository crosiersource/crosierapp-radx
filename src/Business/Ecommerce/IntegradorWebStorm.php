...
class IntegradorWebStorm implements IntegradorEcommerce
{
    ...
    public function integraProduto(Produto $produto, ?bool $integrarImagens = true, ?bool $respeitarDelay = false, ?bool $reintegrarRecursivo = true): void
    {
        ...
        // na verdade, na rodoponta tudo o que vai para a webstorm eles já mandam para o ML
        // portanto, o jsonData['mercado_livre']='S' significa que o produto está integrado no site
        $produto->jsonData['mercado_livre'] = 'S';
    ...
    public function atualizaEstoqueEPrecos(array $produtosIds): void
    {
        try {
            ...
            $this->syslog->info('atualizaEstoqueEPrecos - XML RESPONSE - ', $arResultado);
    ...
    public function obterVendasPorData(\DateTime $dtVenda)
    {
        ...
        $this->syslog->info('RETORNO: Obtendo vendas entre ' . $dtIni->format('d/m/Y H:i:s') . ' e ' . $dtFim->format('d/m/Y H:i:s'), utf8_encode($arResultado));
            ...
            if ($pedido->entrega->retirarLoja ?? false) {
                if ((int)$pedido->entrega->retirarLoja->__toString() === 1) {
                    ...
                    if ($pedido->entrega->agendamento ?? false) {
                        $dtAgendamentoEntrega = DateTimeUtils::parseDateStr($pedido->entrega->agendamento->__toString());
                        try {
                            if (!$dtAgendamentoEntrega) {
                                $obs[] = 'Sem agendamento de entrega';
                            } else {
                                $obs[] = 'Agendado para: ' . $dtAgendamentoEntrega->format('d/m/Y H:i');
                            }
                        } catch (\Throwable $e) {
    ...
    {
        // Verifica se a string é nula ou tem menos de 7 caracteres
        if (is_null($string) || strlen($string) < 7) {
            return '0000000000';
        }
